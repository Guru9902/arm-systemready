...
get_sysarch_acs_src()
{
    ...
    if [ -z $SYS_ARCH_ACS_TAG ]; then
        ...
        git clone --depth 1 https://github.com/Sujana-M/sysarch-acs.git ShellPkg/Application/sysarch-acs
    else
        echo "Downloading Arm BSA source code. TAG : $SYS_ARCH_ACS_TAG"
        git clone --depth 1 --branch $SYS_ARCH_ACS_TAG https://github.com/Sujana-M/sysarch-acs.git ShellPkg/Application/sysarch-acs
    fi
    popd
